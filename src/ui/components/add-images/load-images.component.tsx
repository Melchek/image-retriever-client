...
class LoadImagesModalComponent extends Component<LoadImagesProps, any> {
    ...
    onFinish = async () => {
        ...
        try {
            await this.props.mainStore!.uploadImages(this.imageFile!, this.queryImageFile!, this.image, this.queryImage)
            this.props.onAdded()
        } catch (err) {
            console.error(err)
        } finally {
    ...
    onFilesAdded = (step: StepsEnum, files: File[]) => {
        ...
        files.forEach((file: File, index: number) => {
            ...
            readers[index].addEventListener('load', action(async () => {
                ...
                switch (step) {
                    case StepsEnum.image:
                        this.isLoading = true
                        this.image = await compressToDataUrl(readers[index].result as string)
                        this.isLoading = false
                        this.imageFile = file
                        break;
                ...
                return <div className='logo-container'>
                    {this.image ?
                        <React.Fragment>
                            ...
                            <Button onClick={this.reloadImage.bind(this, StepsEnum.image)}>{t('reloadImage')}</Button>
                        </React.Fragment>
                        :
                        <DropZoneComponent onFilesAdded={this.onFilesAdded.bind(this, StepsEnum.image)} />
                    }
                </div>
            case StepsEnum.queryImage:
                return <div className='cover-logo-container'>
                    {this.queryImage ?
                        <React.Fragment>
                            ...
                            <Button onClick={this.reloadImage.bind(this, StepsEnum.queryImage)}>{t('reloadImage')}</Button>
                        </React.Fragment>
                        :
                        <DropZoneComponent onFilesAdded={this.onFilesAdded.bind(this, StepsEnum.queryImage)} />
                    }
                </div>
            default:
                break;
    ...
    render() {
        ...
        const { uploadingPercentCompleted, isLoadingImages, isProcessing } = mainStore!

        return (
            <Dialog onClose={this.onCancel} aria-labelledby='customized-dialog-title'
                open={isLoadingImages} className='add-images'>
                <DialogContent className='dialog-content'>
                    {!!uploadingPercentCompleted || isProcessing ?
                        <React.Fragment>
                            {!!uploadingPercentCompleted && <LinearProgress value={uploadingPercentCompleted} />}
                        ...
                        <React.Fragment>
                            ...
                            <div className='buttons'>
                                ...
                                >
                                    {this.activeStep === this.steps.length - 1 ? t('finish') : t('next')}
                                </Button>
                            </div>
                        </React.Fragment>
                    }
